...
void FindMeshInfo(const aiScene* scene, aiNode* node, FTransform& ParentTransform, FTransform& WorldTransform, FReturnedData& result)
{
	...
	FTransform NodeTransform = FTransform(tempMatrix);
	FTransform::Multiply(&WorldTransform, &NodeTransform, &ParentTransform);
	for (uint32 i = 0; i < node->mNumMeshes; i++)
	{
		...
		FMeshInfo &mi = result.MeshInfoArray[meshidx];
		...
		mi.MaterialName = FString(name.C_Str());

		mi.RelativeTransform = NodeTransform;
...
void FindMesh(const aiScene* scene, aiNode* node, FTransform& ParentTransform, FReturnedData& retdata)
{
	FTransform WorldTransform;
	FindMeshInfo(scene, node, ParentTransform, WorldTransform, retdata);

	//int MeshIndex = *node->mMeshes;
	//FMeshInfo& MeshInfo = retdata.MeshInfoArray[MeshIndex];

	for (uint32 m = 0; m < node->mNumChildren; ++m)
	{
		FindMesh(scene, node->mChildren[m], WorldTransform, retdata);
	}
}
...
FReturnedData ULoaderBPFunctionLibrary::LoadMesh(FString filepath, EPathType type)
{
	...
	result.MeshInfoArray.Empty();
	result.NumMeshes = 0;
	//bEnableCollision:: StaticMeshSection
	//result.MeshInfoArray.Vertices.Empty();
	//result.MeshInfoArray.Triangles.Empty();
	//result.MeshInfoArray.Normals.Empty();
	//result.MeshInfoArray.UV0.Empty();
	//result.MeshInfoArray.VertexColors.Empty();
	//result.MeshInfoArray.Tangents.Empty();
	...
	case EPathType::Relative:
		file = TCHAR_TO_UTF8(*FPaths::Combine(FPaths::ProjectDir(), filepath));
		break;
	}
	...
	{
		result.MeshInfoArray.SetNum(mScenePtr->mNumMeshes, false);

		FTransform RootTransform = FTransform::Identity;
		FindMesh(mScenePtr, mScenePtr->mRootNode, RootTransform, result);
		...
		for (uint32 i = 0; i < mScenePtr->mNumMeshes; ++i)
		{
			...
			for (uint32 l = 0; l < mScenePtr->mMeshes[i]->mNumFaces; ++l)
			{
				...
				{
					result.MeshInfoArray[i].Triangles.Push(mScenePtr->mMeshes[i]->mFaces[l].mIndices[m]);
				}
			}
		}
