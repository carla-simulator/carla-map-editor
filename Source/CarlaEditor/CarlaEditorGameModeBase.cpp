...
ACarlaEditorGameModeBase::ACarlaEditorGameModeBase(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
        ...
        for (auto& MeshInfo : Data.MeshInfoArray)
        {
            ...
            for (const TPair<FString, UMaterial*>& pair : NameMaterialMap)
            {
                ...
                if (MeshInfo.MaterialName.Contains(pair.Key, ESearchCase::IgnoreCase))
                {
                    ...
                    AProceduralActor* ProceduralActor = GetWorld()->SpawnActor<AProceduralActor>(AProceduralActor::StaticClass(), FVector::ZeroVector, FRotator::ZeroRotator);
...
TArray<FString> ACarlaEditorGameModeBase::GetAllFilesInDirectory(const FString Directory, const FString OnlyFilesStartingWith, const FString OnlyFilesWithExtension)
{
